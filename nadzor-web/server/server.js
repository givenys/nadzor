...
const incidentViewers = new Set();

global.wss = wss;
global.incidentViewers = incidentViewers;
...
wss.on('connection', (ws, req) => {
  ...
  // Зритель (оператор) - видеопоток
  else if (wsPath === '/api/cams/viewer') {
    if (!token) {
      console.log('Viewer WS отклонен: нет токена');
    ...
    try {
      ...
      ws.user = decoded;
      console.log(`Зритель ${decoded.login} подключился к камере ${deviceId}`);
    } catch (err) {
      console.log('Viewer WS отклонен: невалидный токен');
  ...
  // ===== Зритель инцидентов =====
  else if (wsPath === '/api/cams/incidents/ws') {
    if (!token) {
      console.log('Incidents WS отклонен: нет токена');
      ws.close(4004, 'No token');
      return;
    }
    ...
    try {
      const decoded = jwt.verify(token, JWT_SECRET);
      ws.user = decoded;
      incidentViewers.add(ws);
      console.log(`Зритель инцидентов ${decoded.login} подключился`);
    } catch (err) {
      console.log('Incidents WS отклонен: невалидный токен');
      ws.close(4005, 'Invalid token');
      return;
    }

    ws.on('close', () => {
      incidentViewers.delete(ws);
      console.log('Зритель инцидентов отключился');
    });
  }
  else {
    ws.close();
